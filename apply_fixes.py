#!/usr/bin/env python3
import re,os,sys,base64,json
import requests

TOKEN_PATH='/tmp/installation.token'
if not os.path.exists(TOKEN_PATH):
    print('Missing token at', TOKEN_PATH); sys.exit(1)
TOKEN=open(TOKEN_PATH).read().strip()
HEADERS={'Authorization':f'Bearer {TOKEN}','Accept':'application/vnd.github+json'}
OWNER='Fintech-Blueprint'; REPO='example-service'; BRANCH='feature/add-ping-endpoint'
...
def put_file(path, localfile, message, branch):
    ...
    if resp.status_code not in (200,201):
        print('Failed to put', path, resp.status_code, resp.text); sys.exit(1)
